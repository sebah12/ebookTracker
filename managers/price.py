...
class PriceManager:
    ...
    @staticmethod
    async def get_lowest_item_price(item_id):
        """Get the lowest historic price (most recent) for the given item."""
        q = price.select()
        q = q.where(
            price.c.item_id == item_id
        ).order_by(
                price.c.price, price.c.date.desc()
        )
        return await database.fetch_one(q)
        return await database.fetch_one(q)
